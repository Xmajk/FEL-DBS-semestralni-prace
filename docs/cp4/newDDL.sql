...
-- Manazer je specializací Zamestnanec (JPA JOINED inheritance).
-- id_zamestnanec je PK i FK na Zamestnanec — sdílené ID vyžadované strategií JOINED.
-- Spravované oddělení je určeno zděděným id_oddeleni ze Zamestnanec;
-- pravidlo "jeden manažer na oddělení" garantuje SERIALIZABLE transakce ve vymenManazera.
CREATE TABLE "Manazer" (
    ...
    CONSTRAINT "Ref_Manazer_to_Zamestnanec" FOREIGN KEY ("id_zamestnanec") REFERENCES "Zamestnanec"("id_zamestnanec") MATCH SIMPLE ON DELETE CASCADE ON UPDATE CASCADE NOT DEFERRABLE
);

-- Externista je specializací Zamestnanec (JPA JOINED inheritance).
-- id_zamestnanec je PK i FK na Zamestnanec — sdílené ID vyžadované strategií JOINED.
